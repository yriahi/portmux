...
**Domain:** Minimal Docker stub/mock HTTP server, multi-port, JSON response
...
**If you need a non-root user at runtime (Kubernetes SecurityContext enforces runAsNonRoot):**
...
**If you want TLS in the future (out of scope per PROJECT.md but possible later):**
...
**If the binary must pass Kubernetes liveness/readiness probes on the same ports:**
